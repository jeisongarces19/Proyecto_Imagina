...
import DWGOverlay2D from './DWGOverlay2D';
...
export default function Plan2DOverlay({
  ...
  selectedIds = [], // ✅ selección múltiple (instanceId/uuid)
  onPickIds, // ✅ (ids[]) => void
  onPickId, // compat: (id) => void
  ...
  // Layout (IMPORTANTE: con default)
  width = '100%', // ✅ por defecto grande
  height = 220, // ✅ alto tipo “barra inferior”
  ...
  // Si el 2D te queda “al revés”, cambia a false
  invertZ = true, // ✅ con esto alineas 2D vs 3D
  plan2DSrc,
  plan2DVisible = true,
}) {
  ...
  const handleMouseMove = useCallback(
    ...
    [wallMode, canvasToWorld, invertZ]
  );
  ...
  const handleClick = useCallback(
    (e) => {
      ...
      // compat: si alguien escucha onPickId, le mandamos el “último”
      onPickId?.(best.id);
    },
    [
      wallMode,
  ...
  useEffect(() => {
    const onKey = (ev) => {
      ...
      if (ev.key === 'Enter') {
        if (wallMode) commitWall();
      }
      if (ev.key?.toLowerCase?.() === 'f') fitView(); // F = fit
    };
    window.addEventListener('keydown', onKey);
    return () => window.removeEventListener('keydown', onKey);
  }, [wallMode, commitWall, clearDraft, fitView]);
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // helpers inline (evita depender de toCanvas del closure)
      const toCanvasLocal = (x, z) => {
        const px = (x - cx) * s + w / 2;
        const py = -(z - cz) * s + h / 2; // ✅ Z+ hacia arriba en canvas
        return [px, py];
      };
      ...
      if (wallMode && (draftPts?.length || 0) > 0) {
        const pts = [...draftPts];
    ...
    >
      ...
      >
        ...
        {plan2DSrc && (
          <DWGOverlay2D
            canvasRef={canvasRef}
            viewRef={viewRef}
            src={plan2DSrc} // ✅ url del file local
            opacity={0.25}
            scale={1}
            offset={{ x: 0, z: 0 }}
      ...
      <canvas
        ...
        style={{
          ...
          height: '100%',
          display: 'block',
          cursor: wallMode ? 'crosshair' : 'pointer',
        }}
        onClick={handleClick}
